#!/usr/bin/env python3
"""
MLP2 Hyperparameter Experiment Runner
=====================================
...
MLP2_SCRIPT = os.path.join(BASE_DIR, 'MLP2.py')
...
def backup_original_mlp2():
    """Create backup of original MLP2.py configuration before modifications.
    ...
    """
    backup_path = os.path.join(BASE_DIR, 'MLP2_original_backup.py')
    if not os.path.exists(backup_path):
        shutil.copy2(MLP2_SCRIPT, backup_path)
        print(f"✓ Backed up original MLP2.py to {backup_path}")
    else:
        print(f"✓ Backup already exists: {backup_path}")

def modify_mlp2_config(feature_mode, arch_name, lr, wd, batch_size):
    """Modify MLP2.py configuration parameters for current experiment.
    ...
    with open(MLP2_SCRIPT, 'w') as f:
        f.write('\n'.join(modified_lines))

def restore_original_mlp2():
    """Restore the original MLP2.py configuration from backup.
    ...
    if os.path.exists(backup_path):
        shutil.copy2(backup_path, MLP2_SCRIPT)
        print("✓ Restored original MLP2.py configuration")
    else:
...
def run_single_experiment(feature_mode, arch_name, lr, wd, batch_size, exp_num, total_exp):
    ...
    try:
        ...
        modify_mlp2_config(feature_mode, arch_name, lr, wd, batch_size)
        
        # Execute MLP2 training
        result = subprocess.run([
            sys.executable, MLP2_SCRIPT
...
def main():
    ...
    print("=" * 80)
    print("MLP2 HYPERPARAMETER OPTIMIZATION SUITE")
    print("=" * 80)
