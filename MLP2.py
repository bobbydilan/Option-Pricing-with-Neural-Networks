...
# Normalized target training parameters
NORMALIZED_EPOCHS = 200
NORMALIZED_LEARNING_RATE = 3e-4
...
USE_NORMALIZED_TARGET = True    # Train on price/underlying ratio for better generalization
USE_ZSCORE_SCALING = False      # Apply Z-score normalization to targets
USE_VEGA_WEIGHTED_LOSS = True   # Weight loss by option vega for sensitivity-aware training
HUBER_DELTA = 0.05 if USE_NORMALIZED_TARGET else 1.0
USE_KAIMING_INIT = False        # Weight initialization method
USE_ADAMW = True                # Optimizer selection

# Sample weighting for balanced training across moneyness and time dimensions
USE_MONEYNESS_SQRT_T_WEIGHTING = True
MONEYNESS_BINS = 7
SQRT_T_BINS = 7
MIN_SQRT_T = 0.0
MAX_SQRT_T = np.sqrt(2)
MIN_MONEYNESS = 0.5
MAX_MONEYNESS = 1.5
DEBUG_SAMPLE_WEIGHTING = True

# Data quality filters
MAX_SPREAD_RATIO = 100          # Legacy spread filter
SPREAD_FILTER_ENABLED = False   # Enable percentage-based spread filtering
MIN_SPREAD_PCT = None
MAX_SPREAD_PCT = None

# Evaluation bucket definitions for consistent analysis across functions
...
FEATURE_MODE = 'simplified_plus_all'  # Options: 'core_only', 'default_only'  or MLP2:   'simplified_only', 'simplified_plus_all', 'simplified_plus_non_overlap'
...
USE_MULTI_DIM_STRATIFICATION = True  # Stratify by price × moneyness × time buckets
...
def _collect_run_config() -> dict:
    ...
    cfg = dict(
        ...
        EPOCHS=EPOCHS if not USE_NORMALIZED_TARGET else NORMALIZED_EPOCHS,
        BATCH_SIZE=BATCH_SIZE,
        LEARNING_RATE=LEARNING_RATE if not USE_NORMALIZED_TARGET else NORMALIZED_LEARNING_RATE,
...
def plot_true_vs_predicted(y_true, y_pred, results_dir):
...
def plot_residuals_vs_features(y_true, y_pred, df_test_or_X_original, results_dir):
    ...
    # 2. Average Absolute Residuals vs Moneyness (binned bar chart)
    plt.subplot(2, 2, 2)
    moneyness = spx_close / strike_price  # S/K
    ...
    for bucket in MONEYNESS_BIN_LABELS:
        ...
        if mask.sum() > 0:
            bucket_errors.append(np.mean(np.abs(residuals[mask])))
            bucket_labels.append(bucket)
            bucket_counts.append(mask.sum())
    
    plt.bar(bucket_labels, bucket_errors, color='lightblue', alpha=0.7)
    plt.xlabel('Moneyness Buckets', fontsize=12)
    plt.ylabel('Average Absolute Residuals ($)', fontsize=12)
    plt.title('Average Absolute Residuals vs Moneyness', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    
    # 3. Average Absolute Residuals vs Days to Maturity (binned bar chart)
    plt.subplot(2, 2, 3)
    time_to_expiration = days_to_maturity / 365.25
    time_buckets = get_time_buckets(time_to_expiration)
    ...
    for bucket in TIME_BIN_LABELS:
        ...
        if mask.sum() > 0:
            time_errors.append(np.mean(np.abs(residuals[mask])))
            time_labels.append(bucket)
            time_counts.append(mask.sum())
    
    plt.bar(time_labels, time_errors, color='orange', alpha=0.7)
    plt.xlabel('Time to Expiration Buckets', fontsize=12)
    plt.ylabel('Average Absolute Residuals ($)', fontsize=12)
    plt.title('Average Absolute Residuals vs Time to Expiration', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
...
def plot_error_by_features(y_true, y_pred, df_test_or_X_original, results_dir):
    """Create error analysis by features - Third PNG"""
    y_pred = _clip_eval_predictions(y_pred)
    ...
    plt.subplot(2, 2, 1)
    quantiles = pd.qcut(y_true, q=5, labels=False, duplicates='drop')
    
    error_by_quantile = []
    quantile_labels = []
    
    for q in range(len(np.unique(quantiles))):
        mask = quantiles == q
        if mask.sum() > 0:
            error_by_quantile.append(np.mean(abs_residuals[mask]))
    ...
    for bar, count in zip(bars1, quantile_labels):
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2., height + height*0.01,
                f'n={count}', ha='center', va='bottom', fontsize=9, fontweight='bold')
    
    # 2. Error by Moneyness (use centralized buckets)
    plt.subplot(2, 2, 2)
    moneyness = spx_close / strike_price  # S/K
    moneyness_buckets = get_moneyness_buckets(moneyness)
    ...
    # 3. Error by Time to Maturity (use centralized time buckets)
    plt.subplot(2, 2, 3)
    time_to_expiration = days_to_maturity / 365.25
    time_buckets = get_time_buckets(time_to_expiration)
    ...
    # 4. Error by Historical Volatility (Bar Chart with Bounds)
    plt.subplot(2, 2, 4)
    # Robust quantile bucketing: duplicates='drop' can reduce bins; derive labels accordingly
    vol_q_indices, vol_q_bins = pd.qcut(
    ...
    # Determine actual number of bins after dropping duplicates
    n_vol_bins = max(0, len(vol_q_bins) - 1)
    vol_label_catalog = ['Low Vol', 'Med-Low Vol', 'Med-High Vol', 'High Vol']
    vol_label_catalog = vol_label_catalog[:n_vol_bins]
    ...
    for i in range(n_vol_bins):
        vol_label = vol_label_catalog[i]
        mask = (vol_q_indices == i)
        if mask.sum() > 0:
            vol_errors.append(abs_pct_error[mask].mean())
            vol_labels.append(vol_label)
            vol_min = historical_volatility[mask].min()
            vol_max = historical_volatility[mask].max()
...
Price Range: ${y_true.min():.2f} - ${y_true.max():.2f}"""
    ...
    plt.subplot(3, 3, 6)
    input_layer_size = len(DEFAULT_FEATURES)
...
def get_lr_with_warmup(epoch, warmup_epochs=5):
    """Calculate learning rate with linear warmup for improved training stability.
...
def calculate_black_scholes_vega(S, K, r, q, T, sigma):
    ...
    T = np.maximum(T, 1.0 / 365.0)  # Minimum 1 day
...
def plot_error_pivot_grid(y_true, y_pred, df_test_or_X_original, results_dir):
    ...
    time_to_expiration = days_to_maturity / 365.25
    den = np.maximum(np.abs(y_true), EPS)
    abs_pct_error = np.abs((y_pred - y_true) / den) * 100
    ...
    for i in range(len(time_bins)-1):
        for j in range(len(moneyness_bins)-1):
            ...
            if combined_mask.sum() >= 10:  # Minimum sample size
                pivot_data[i, j] = np.mean(abs_pct_error[combined_mask])
...
def compute_moneyness_sqrt_t_weights(df):
    ...
    sqrt_t = np.sqrt(df['days_to_maturity'].values / 365.0)  # Convert days to years, then sqrt
    ...
    # --- Diagnostics: how many unique bins are actually used? ---
    if 'DEBUG_SAMPLE_WEIGHTING' in globals() and DEBUG_SAMPLE_WEIGHTING:
        used_pairs = np.unique(np.stack([moneyness_bin_indices, sqrt_t_bin_indices], axis=1), axis=0)
        print(f"Unique (moneyness, √T) bins used: {used_pairs.shape[0]} / {MONEYNESS_BINS * SQRT_T_BINS}")
    ...
    # Bin count stats diagnostics
    if 'DEBUG_SAMPLE_WEIGHTING' in globals() and DEBUG_SAMPLE_WEIGHTING:
        nonempty = bin_counts[bin_counts > 0]
        if nonempty.size > 0:
            print(f"Bin count stats -> min:{nonempty.min():.0f} mean:{nonempty.mean():.1f} max:{nonempty.max():.0f}")
        else:
    ...
    mean_count = bin_counts[valid_bins].mean() if np.any(valid_bins) else 1.0

    # Create weight lookup table based on relative inverse frequency
    weight_lookup = np.zeros_like(bin_counts, dtype=float)
    weight_lookup[valid_bins] = mean_count / bin_counts[valid_bins]
    ...
    # Diagnostics: unique raw weights before clip/renorm
    if 'DEBUG_SAMPLE_WEIGHTING' in globals() and DEBUG_SAMPLE_WEIGHTING:
        uniq_w = np.unique(sample_weights)
        if uniq_w.size <= 1:
            print("Note: All samples mapped to the same 2D bin (or equally populated bins).\n"
                  "      This will produce uniform weights (range 1.0–1.0 after renorm). Consider: \n"
                  "      • increasing MONEYNESS_BINS / SQRT_T_BINS, \n"
                  "      • widening MIN/MAX bounds, or \n"
                  "      • computing weights from a different density proxy (e.g., vega).")
    
    # Improved clipping with tighter bounds and batch renormalization
    # Use more conservative clipping similar to vega weighting
    w_min, w_max = 0.5, 2.0  # Tight clipping bounds
    ...
    # Final diagnostics
    if 'DEBUG_SAMPLE_WEIGHTING' in globals() and DEBUG_SAMPLE_WEIGHTING:
        print(f"Sample weights std (post-renorm): {np.std(sample_weights):.6f}")
...
class VegaWeightedHuberLoss(nn.Module):
    ...
    def forward(self, predictions, targets, vega_weights):
        ...
        # Apply ATM emphasis weighting with square root to avoid extreme weights
        vmin = 1e-3 if USE_NORMALIZED_TARGET else 0.1
        w = torch.clamp(vega_weights, min=vmin).pow(0.5)
        
        # Clip weights and renormalize to maintain training stability
        w = torch.clamp(w, min=0.5, max=2.0)
        w = w / torch.mean(w)
...
def _get_input_size_for_feature_mode():
    ...
    if FEATURE_MODE == 'simplified_only':
        return 2
    elif FEATURE_MODE == 'simplified_plus_all':
        return 2 + len(DEFAULT_FEATURES)
    elif FEATURE_MODE == 'default_only':
        return len(DEFAULT_FEATURES)
    elif FEATURE_MODE == 'simplified_plus_non_overlap':
        return 2 + len(ADDITIONAL_FEATURES)
    elif FEATURE_MODE == 'core_only':
...
def _create_feature_matrix(df):
    ...
    T_years = np.maximum(df['days_to_maturity'].values, 0) / 365.25
    ...
    if FEATURE_MODE == 'simplified_only':
        X = np.column_stack([sqrt_T, fwd_log_moneyness])
        feature_names = ['sqrt_T', 'fwd_log_moneyness']
        
    elif FEATURE_MODE == 'simplified_plus_all':
        # Simplified features + all default features
        simplified_features = np.column_stack([sqrt_T, fwd_log_moneyness])
        default_features = df[DEFAULT_FEATURES].values
        X = np.column_stack([simplified_features, default_features])
        feature_names = ['sqrt_T', 'fwd_log_moneyness'] + DEFAULT_FEATURES
        
    elif FEATURE_MODE == 'default_only':
        # Only default features (no simplified)
        X = df[DEFAULT_FEATURES].values
        feature_names = DEFAULT_FEATURES
        
    elif FEATURE_MODE == 'simplified_plus_non_overlap':
        # Simplified features + features not used in their calculation
        simplified_features = np.column_stack([sqrt_T, fwd_log_moneyness])
        additional_features = df[ADDITIONAL_FEATURES].values
        X = np.column_stack([simplified_features, additional_features])
        feature_names = ['sqrt_T', 'fwd_log_moneyness'] + ADDITIONAL_FEATURES
        ...
        raise ValueError(f"Invalid FEATURE_MODE: {FEATURE_MODE}. Must be one of: 'simplified_only', 'simplified_plus_all', 'default_only', 'simplified_plus_non_overlap', 'core_only'")
...
def get_moneyness_buckets(moneyness: np.ndarray):
    """Bucketize moneyness using centralized edges/labels."""
    return pd.cut(moneyness, bins=MONEYNESS_BINS_EDGES, labels=MONEYNESS_BIN_LABELS)

def get_time_buckets(time_in_years: np.ndarray):
    """Bucketize time-to-expiration (years) using centralized edges/labels."""
    return pd.cut(time_in_years, bins=TIME_BINS_EDGES, labels=TIME_BIN_LABELS)
...
def calculate_bucketed_metrics(y_true, y_pred, df_test_or_X_original, results_dir):
    ...
    moneyness = spx_close / strike_price
    time_to_expiration = days_to_maturity / 365.25  # Convert to years
    
    # Create buckets using centralized definitions
    moneyness_buckets = get_moneyness_buckets(moneyness)
    time_buckets = get_time_buckets(time_to_expiration)
    
    # Calculate metrics by moneyness buckets
    print(f"\n--- Bucketed Metrics by Moneyness ---")
    moneyness_metrics = {}
    for bucket in moneyness_buckets.categories:
        mask = moneyness_buckets == bucket
        if mask.sum() > 0:
            bucket_mae = np.mean(abs_residuals[mask])
            bucket_mape = np.mean(abs_pct_error[mask])
            ...
            print(f"  {bucket}: MAE=${bucket_mae:.2f}, MAPE={bucket_mape:.1f}%, n={bucket_count:,}")
    
    # Calculate metrics by time buckets
    print(f"\n--- Bucketed Metrics by Time to Maturity ---")
    time_metrics = {}
    for bucket in time_buckets.categories:
        mask = time_buckets == bucket
        if mask.sum() > 0:
            bucket_mae = np.mean(abs_residuals[mask])
            bucket_mape = np.mean(abs_pct_error[mask])
...
def create_random_splits(X, y, indices, df_original=None):
    ...
    # Step 1: Create df_view exactly like Black-Scholes
    # Black-Scholes uses: df_view = df.reset_index(drop=True)
    # We need to use the subset of df_original at indices, then reset_index
...
# Core features used for simplified feature calculation
CORE_FEATURES = [
    "strike_price",
    "dividend_rate",
    "risk_free_rate", 
...
# Additional features not used in simplified feature calculation
ADDITIONAL_FEATURES = [
    "volume",
    "epu_index",
    "open_interest",
    "equity_uncertainty",
    "equity_volatility",
    "historical_volatility" # must be added because not in moneyness
]
...
# Complete default feature set (for backward compatibility)
DEFAULT_FEATURES = CORE_FEATURES + ADDITIONAL_FEATURES
...
CURRENT_FEATURE_NAMES = list(DEFAULT_FEATURES)  # will be updated where X is constructed
...
def load_dataset(csv_path: str, sample_size: int = 1000000):
    ...
    required_columns = set(DEFAULT_FEATURES + [TARGET_COLUMN])
    
    # Note: bs_vega is computed from scratch using historical volatility, not loaded from dataset
    
    # Moneyness × √T weighting uses standard columns (no additional requirements)
    # if USE_MONEYNESS_SQRT_T_WEIGHTING: - no extra columns needed
    ...
    # Zero-volume filtering moved to after invalid values filter to match Black-Scholes order
    ...
    for label in price_bins.cat.categories:
    ...
    if USE_VEGA_WEIGHTED_LOSS:
        ...
        if all(col in df.columns for col in ['spx_close', 'strike_price', 'risk_free_rate', 'dividend_rate', 'days_to_maturity', 'historical_volatility']):
            ...
            # Use consolidated Black-Scholes vega calculation
            S = df['spx_close'].values
            K = df['strike_price'].values
            r = df['risk_free_rate'].values
            q = df['dividend_rate'].values
...
def train():
    ...
    # Add verification prints to confirm matching splits with Black-Scholes
    ...
    # Save test identifiers for cross-script verification (matching Black-Scholes)
    ...
    current_epochs = NORMALIZED_EPOCHS if USE_NORMALIZED_TARGET else EPOCHS
    current_lr = NORMALIZED_LEARNING_RATE if USE_NORMALIZED_TARGET else LEARNING_RATE
        ...
        for batch_data in train_loader:
            ...
            if USE_VEGA_WEIGHTED_LOSS and vega_b is not None:
                ...
                if USE_MONEYNESS_SQRT_T_WEIGHTING and sample_b is not None:
                    # Scale by sample weights (moneyness × √T)
    ...
    plot_true_vs_predicted(trues, preds, results_dir)
    ...
    plot_error_by_features(trues, preds, df_test, results_dir)
    ...
    # Use centralized bucket definitions
    moneyness_buckets = pd.cut(moneyness, bins=MONEYNESS_BINS_EDGES, labels=MONEYNESS_BIN_LABELS)
    time_buckets = pd.cut(time_to_expiration, bins=TIME_BINS_EDGES, labels=TIME_BIN_LABELS)
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"Moneyness Bounds: {MONEYNESS_LOWER_BOUND} to {MONEYNESS_UPPER_BOUND}\n")
        ...
        # Prediction Error Statistics (comparable to OLS/Black-Scholes)
        ...
        for bucket in ['Deep OTM', 'OTM', 'ATM', 'ITM', 'Deep ITM']:
            ...
            if mask.sum() > 0:
                ...
                bucket_mae = np.mean(abs_residuals[mask])
                bucket_mape = np.mean(abs_pct_error[mask])
                bucket_rmse = np.sqrt(np.mean(residuals[mask]**2))
                bucket_mean_error = np.mean(residuals[mask])
                bucket_median_abs_error = np.median(abs_residuals[mask])
                bucket_spread_accuracy = np.mean(in_spread[mask]) * 100
                
                f.write(f"{bucket:>10} (n={bucket_count:,}):\n")
                f.write(f"  Mean Absolute Error: ${bucket_mae:.4f}\n")
                f.write(f"  RMSE: ${bucket_rmse:.4f}\n")
                f.write(f"  Mean Abs % Error: {bucket_mape:.2f}%\n")
                f.write(f"  Mean Error (bias): ${bucket_mean_error:.4f}\n")
                f.write(f"  Median Abs Error: ${bucket_median_abs_error:.4f}\n")
                f.write(f"  Bid-Ask Spread Accuracy: {bucket_spread_accuracy:.2f}%\n\n")
        ...
        for bucket in ['<1M', '1-3M', '3-6M', '6M-1Y', '>1Y']:
            mask = time_buckets == bucket
            if mask.sum() > 0:
                bucket_count = mask.sum()
                bucket_mae = np.mean(abs_residuals[mask])
                ...
                f.write(f"{bucket:>6} (n={bucket_count:,}):\n")
                f.write(f"  Mean Absolute Error: ${bucket_mae:.4f}\n")
                f.write(f"  RMSE: ${bucket_rmse:.4f}\n")
                f.write(f"  Mean Abs % Error: {bucket_mape:.2f}%\n")
                f.write(f"  Mean Error (bias): ${bucket_mean_error:.4f}\n")
