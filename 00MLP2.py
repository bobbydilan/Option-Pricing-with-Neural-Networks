...
DATA_QUALITY_ISSUES = []
FEATURE_MODE = 'simplified_plus_all'               # simplified_only, simplified_plus_selected, simplified_plus_all
...
def write_predictions_with_context(results_dir: str,
                                 df_test_context: pd.DataFrame,
                                 y_true: np.ndarray,
                                 y_pred: np.ndarray,
                                 X_test_original: np.ndarray | None = None,
                                 CURRENT_FEATURE_NAMES: list[str] | None = None) -> str:
    """
    Save mlp_predictions.csv with FULL BSM context + engineered features, regardless of feature mode.
    Also appends the actual model inputs used (as <feature>_input) when provided.
    """
    dfc = df_test_context.copy()

    # Ensure mid_price if bid/ask present
    if 'mid_price' not in dfc.columns and {'best_bid', 'best_offer'} <= set(dfc.columns):
        dfc['mid_price'] = (dfc['best_bid'] + dfc['best_offer']) / 2.0

    # Core BSM inputs
    S = _safe_col(dfc, 'spx_close', default=np.nan)
    K = _safe_col(dfc, 'strike_price', default=np.nan)
    r = _safe_col(dfc, 'risk_free_rate', default=np.nan)
    q = _safe_col(dfc, 'dividend_rate', default=np.nan)
    days = _safe_col(dfc, 'days_to_maturity', default=np.nan)
    T = np.maximum(days, 0) / DAYS_PER_YEAR

    with np.errstate(divide='ignore', invalid='ignore'):
        F = S * np.exp((r - q) * T)
        moneyness = np.where(K > 0, S / K, np.nan)
        fwd_log_moneyness = np.where((F > 0) & (K > 0), np.log(F / K), np.nan)
    sqrt_T = np.sqrt(np.maximum(T, 0))

    # Start output with identifiers/market + core BSM if present
    cols_ident = [c for c in ['date','option_id','root','expiration','cp_flag','option_type'] if c in dfc.columns]
    cols_market = [c for c in ['best_bid','best_offer','mid_price','volume','open_interest'] if c in dfc.columns]
    cols_bsm = [c for c in ['spx_close','strike_price','risk_free_rate','dividend_rate','days_to_maturity','historical_volatility'] if c in dfc.columns]

    out = pd.DataFrame(index=dfc.index)
    for c in cols_ident + cols_market + cols_bsm:
        out[c] = dfc[c]
    ...
    out['fwd_log_moneyness'] = fwd_log_moneyness

    # Targets & errors (unnormalized dollars)
    out['true_price'] = np.asarray(y_true)
    out['predicted_price'] = np.asarray(y_pred)
    residual = out['predicted_price'] - out['true_price']
    out['residual'] = residual
    out['abs_residual'] = np.abs(residual)
    denom = np.maximum(np.abs(out['true_price']), EPS)
    out['pct_error'] = 100.0 * residual / denom
    out['abs_pct_error'] = np.abs(out['pct_error'])

    # Include the actual model inputs for auditability (optional)
    if X_test_original is not None and CURRENT_FEATURE_NAMES is not None:
        for i, feature in enumerate(CURRENT_FEATURE_NAMES):
            colname = f'{feature}_input'
            if colname not in out.columns and i < X_test_original.shape[1]:
                out[colname] = X_test_original[:, i]

    csv_path = os.path.join(results_dir, 'mlp_predictions.csv')
    out.to_csv(csv_path, index=False)
    print(f"Predictions with full context saved to {csv_path}")
    return csv_path
...
def export_predictions_csv(df_original, test_indices, X_test_original, y_test_original, predictions, results_dir):
    ...
    # Export predictions with full BSM context (works for simplified modes too)
    csv_path = write_predictions_with_context(
    ...
    df_test = pd.read_csv(csv_path)  # Read the saved CSV to get the full data
    mean_abs_error = df_test['abs_residual'].mean()
    mean_pct_error = df_test['abs_pct_error'].mean()
